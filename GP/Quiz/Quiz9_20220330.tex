...
\begin{itemize}
  ...
  용수철의 탄성 위치에너지로 전환되었다. 따라서,
  \begin{align}\label{eq:1-1}
    E_i = mgd\sin{\theta} = \frac{1}{2}kx^2,\,\,\, x = 5.50\,\mathrm{cm}
    =5.50\times 10^{-2}\mathrm{m}.
  \end{align}
  용수철은 270 N 의 힘을 받을 때 2.00 cm 만큼 압축되므로 용수철 상수 $k$는,
  \begin{align}
...
%      node [below=45,right=-2.5] {$30^\circ$};
  ...
  \end{align}
  따라서 $x_{cm}$은 0 이다.
%  질소 대 수소의 원자질량 비율이 13.9 이므로,
%  \begin{align}
%    \frac{m_N}{m_H} = 13.9.
%  \end{align}
%  따라서 $x_{cm}$은 다음과 같이 구할 수 있다.
%  \begin{align}
%    \begin{split}
%      x_{cm} = \frac{m_{H}x_{H1} + m_{H}x_{H2} + m_{H}x_{H3}}
%      {m_{N} + 3m_{H}}
%      = \frac{d -\frac{1}{2}d -\frac{1}{2}d}
%      {\frac{m_{N}}{m_{H}} + 3} \\
%      =\frac{}{}
%    \end{split}
%  \end{align}

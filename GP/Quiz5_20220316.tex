...
\noindent {\bf 해답} 비행기의 무게는 비행기의 질량에 중력 가속도의 크기를
곱한 값과 같다.
\begin{align}
  W=mg,\,\,\,m=\frac{W}{g}
\end{align}
...
%  $\vec{F}_1$ 과 $\vec{F}_2$ 는 + $x$ 축을 기준으로 각각
%  $180^\circ-\theta_1$ , $270^\circ+\theta_2$ 만큼 벌어져 있으므로,
%  $\vec{F}_1$ 과 $\vec{F}_2$ 의 각 방향 성분은 다음과 같다.
%  \begin{align}
%    \begin{split}
%      \vec{F}_1=|\vec{F}_1|(\cos{(180^\circ-\theta_1)}\,\hat{\bm{i}} +
%      \sin{(180^\circ-\theta_1)}\,\hat{\bm{j}})\,\mathrm{N}\\
%      \vec{F}_2=|\vec{F}_2|(\cos{(270^\circ+\theta_2)}\,\hat{\bm{i}} +
%      \sin{(270^\circ+\theta_2)}\,\hat{\bm{j}})\,\mathrm{N}
%    \end{split}
%  \end{align}
%  따라서, 알짜힘은,
%  \begin{align}
...
%      =&|\vec{F}_1|(\cos{(180^\circ-\theta_1)}\,\hat{\bm{i}} 
%      +\sin{(180^\circ-\theta_1)}\,\hat{\bm{j}})\,\mathrm{N} \\
%      &+|\vec{F}_2|(\cos{(270^\circ+\theta_2)}\,\hat{\bm{i}} 
%      +\sin{(270^\circ+\theta_2)}\,\hat{\bm{j}})\,\mathrm{N} \\
%      =&|\vec{F}_1|(-\cos{\theta_1}\,\hat{\bm{i}} 
%      +\sin{\theta_1}\,\hat{\bm{j}})\,\mathrm{N}
%      +|\vec{F}_2|(\sin{\theta_2}\,\hat{\bm{i}} 
%      -\cos{\theta_2}\,\hat{\bm{j}})\,\mathrm{N}.
%    \end{split}
%  \end{align}
%  $|\vec{F}_1|$는 6.00 이고,
%  $\theta_1=30.0^\circ$, $|\vec{F}_2|$는 7.00,
%  $\theta_2=30.0^\circ$이므로, 알짜힘은 다음과 같다.
%  \begin{align}
%    \begin{split}
%      \sum\vec{F}=&6.00(-\cos{30.0^\circ}\,\hat{\bm{i}} 
%      +\sin{30.0^\circ}\,\hat{\bm{j}})\,\mathrm{N}
%      +7.00(\sin{30.0^\circ}\,\hat{\bm{i}} 
%      -\cos{30.0^\circ}\,\hat{\bm{j}})\,\mathrm{N}  \\
%      =&6.00(-\frac{\sqrt{3}}{2}\,\hat{\bm{i}} 
%      +\frac{1}{2}\,\hat{\bm{j}})\,\mathrm{N}
%      +7.00(\frac{1}{2}\,\hat{\bm{i}} 
%      -\frac{\sqrt{3}}{2}\,\hat{\bm{j}})\,\mathrm{N}  \\
%      =&(-3.00\sqrt{3}\,\hat{\bm{i}} 
%      +3.00\,\hat{\bm{j}})\,\mathrm{N}
%      +(3.50\,\hat{\bm{i}} 
%      -3.50\sqrt{3}\,\hat{\bm{j}})\,\mathrm{N}
%    \end{split}
%  \end{align}
  \item[(나)] 레몬이 일정한 속도 $\vec{v} =(13.0\,\hat{\bm{i}} -
...
\begin{figure}[ht]
  \begin{tikzpicture}[font=\footnotesize]
    ...
    \draw[dashed] (0,0) -- (0,-3) ;
    % Rod + Bob
    \draw (0,0) -- (-45:3) node[fill,circle](m){};
    
    % Weight Force
    \draw[-latex] (m) -- node[right]{$\vec{W}$}++(0,-1) ;

    \draw[-latex] (m) -- node[right]{$\vec{D}$}++(0.5,0) ;
    % Tension Force
    \draw [-latex] (m) -- node[right]{$\vec{T}$}(-45:2);
